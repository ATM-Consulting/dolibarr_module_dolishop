...
class EcmFilesBackward
{
	public static function createCustom($object, $file)
	{
		global $db,$user,$conf;
...
//		{
//			if (! empty($conf->product->enabled)) $upload_dir = $conf->product->multidir_output[$object->entity].'/'.substr(substr("000".$object->id, -2),1,1).'/'.substr(substr("000".$object->id, -2),0,1).'/'.$object->id."/photos";
//			else $upload_dir = $conf->service->multidir_output[$object->entity].'/'.substr(substr("000".$object->id, -2),1,1).'/'.substr(substr("000".$object->id, -2),0,1).'/'.$object->id."/photos";
//		}


		$upload_dir = \Dolishop\DolishopTools::getProductDirScan($object);

		// Define $destfull (path to file including filename) and $destfile (only filename)
		$destfull=$upload_dir . "/" . $file['name'];
		$destfile=$file['name'];
		...
		//$resupload = dol_move_uploaded_file($file['tmp_name'], $destfull, $allowoverwrite, 0, $TFile['error'][$i], 0, $varfiles);
...
//
	...
	public static function deleteCustom($object, $filename)
	{
		global $db,$conf;
...
//		}
		...
		$label = md5_file(dol_osencode($destfull));

		$sql = 'DELETE FROM '.MAIN_DB_PREFIX.'ecm_files WHERE label = \''.$label.'\' AND entity = '.$conf->entity;
		$db->query($sql);
	}
}
...
class EcmFiles //extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (empty($this->position))   // Get max used
		{
			...
			$sql.= " WHERE fullpath ='".$this->db->escape($this->fullpath)."'";
...
//		$sql.= 'gen_or_uploaded,';
		...
		$sql .= ' '.(! isset($this->fullpath)?'NULL':"'".$this->db->escape($this->fullpath)."'").',';
...
//		$sql .= " t.gen_or_uploaded,";
		...
		if ($fullpath) {
			$sql .= " AND t.fullpath = '" . $this->db->escape(dirname($fullpath)) . "' AND t.filename = '".$this->db->escape(basename($fullpath))."'";
		}
		elseif (null !== $ref) {
...
//				$this->gen_or_uploaded = $obj->gen_or_uploaded;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' fullpath = '.(isset($this->fullpath)?"'".$this->db->escape($this->fullpath)."'":"null").',';
